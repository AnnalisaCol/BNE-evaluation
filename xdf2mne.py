...
The module assumes that the timestamps for data and marker streams are synchronized and that the stream's sampling rate matches the
...
def ch_type_transform_default(type_):
    ...
    if type_ in mne.io.get_channel_type_constants():
        return type_
    else:
        return 'misc'
...
def marker_stream2events(marker_stream, t_reference, sfreq, raw, marker_out): # marker_out added by Annalisa
    ...
    markers = np.array(marker_stream['time_series'])[:, marker_out]
